...
def _resolve_entry_timeout(plugin_id: str, entry_id: str) -> float:
    """Resolve timeout from plugin entry metadata; fallback to global default."""
    try:
    ...
    if not isinstance(handlers_snapshot, Mapping):
        return PLUGIN_EXECUTION_TIMEOUT
    ...
    for key in candidate_keys:
        maybe = handlers_snapshot.get(key)
    ...
    timeout_obj = getattr(meta, "timeout", None)
    try:
    ...
    # Guard against pathological values while still honoring entry-level overrides.
...
async def _execute_trigger(
    *,
    ...
    try:
        response = await host.trigger(entry_id, args, timeout=execution_timeout)
...
async def trigger_plugin(
    *,
    ...
    response = await _execute_trigger(
        ...
        args=args,
        trace_id=trace_id,
        execution_timeout=_resolve_entry_timeout(plugin_id, entry_id),
    )

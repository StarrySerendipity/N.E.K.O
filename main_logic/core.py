...
class LLMSessionManager:
    ...
    async def deliver_plugin_answer(self, full_text: str):
        """将插件直连答案通过角色原生 TTS 管线投递到前端。

        用于前端 KB 直连场景：只输出一段插件答案，并保持和常规对话一致的音色。
        """
        text = (full_text or "").strip()
        if not text:
            return

        # 插件答案属于用户触发，允许立即投递（不做 idle 时间限制）。
        can_deliver = await self.prepare_proactive_delivery(min_idle_secs=0.0)
        if not can_deliver:

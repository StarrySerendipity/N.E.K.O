        ...
        textSendButton.addEventListener('click', async function () {
            ...
            if (kbDirectEnabled && text && !hasScreenshots) {
                ...
                try {
                    ...
                    var kbResult = await kbDirect.askDirect(text);
                    var answer = kbResult && kbResult.answer
                        ? kbResult.answer
                        : (window.t ? window.t('agent.status.precheckFailed', { reason: '知识库未返回答案' }) : '知识库未返回答案');

                    await window.ensureWebSocketOpen();
                    if (S.socket && S.socket.readyState === WebSocket.OPEN) {
                        S.socket.send(JSON.stringify({
                            action: 'deliver_plugin_answer',
                            data: answer
                        }));
                    } else {
                    ...
                    window.showStatusToast('知识库问答完成', 2200);
                } catch (error) {
                    window.showStatusToast('知识库问答失败: ' + (error && error.message ? error.message : error), 5000);
                } finally {
                    textSendButton.disabled = false;
                    textInputBox.disabled = false;
                    screenshotButton.disabled = false;
